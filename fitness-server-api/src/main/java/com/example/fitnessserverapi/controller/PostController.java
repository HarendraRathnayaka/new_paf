...
//                posts.addAll(postRepository.findAll());
    ...
    public ResponseEntity<Post> createPosts(
        ...
        try {
            ...
            Post post = new Post(userId,username, title, content,fileNames, false);
    ...
    @PutMapping("/posts/{id}")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    public ResponseEntity<Post> editPosts(
        ...
        try {
            ...
            if (post.isPresent()) {
                ...
                List<String> fileNames = new ArrayList<>(_post.getPostImages());
